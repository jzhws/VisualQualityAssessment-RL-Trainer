...
from modeling_qwen2_5_vl_vllm import Qwen2_5_VLForConditionalGeneration
...
# QUESTION_TEMPLATE = """Answer the question: "[QUESTION]" according to the visual content of this image. Directly provide your score within the <answer> </answer> tags (formatted as Score: b). The final output should be with the following format:
...
class Qwen2VLGRPOVLLMTrainer(Trainer):
    def __init__(
        ...
        # self.accelerator.num_processes=7
    ...
    def _prepare_inputs(
        ...
        if ".mp4" in inputs[0]["video_path"]:
            ...
            prompts = [self.make_conversation_image(example) for example in inputs for _ in range(self.num_generations)]
        ...
        video_inputs = [x["video_inputs"][0][0] for x in inputs if "video_inputs" in x for _ in range(self.num_generations) ]
        # print(len(image_inputs))
        # print(video_inputs[0])
        if video_inputs==[]:
            video_inputs=None
        image_inputs = [x["image_inputs"][0][0] for x in inputs if "image_inputs" in x for _ in range(self.num_generations)]
        # print(len(image_inputs))
        ...
        if image_inputs is None:
            ...
            fps_inputs = [1.0 for _ in range(self.num_generations)]
        ...
        # image_inputs = None
        prompts_text1=copy.deepcopy(prompts_text)
        ...
        if self.args.use_vllm:
            ...
            all_prompts_text = gather_object(prompts_text1)
            ...
            all_multimodal_inputs = [
                {"prompt": p, "multi_modal_data": {"image": v}}
