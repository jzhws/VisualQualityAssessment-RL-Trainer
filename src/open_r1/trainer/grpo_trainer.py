...
# QUESTION_TEMPLATE = """Answer the question: "[QUESTION]" according to the content of the video (A value of 0–1 indicates very poor quality, 1-2 indicates poor quality, 2-3 indicates fair quality, 3-4 indicates good quality, and 4-5 indicates excellent quality.). Firstly, give a through thinking on this  and then output your thought and analysis process within the <think> </think> tags and provide your score within the <answer> </answer> tags (formatted as Score: b). Please note that there is no audio and the audio quality of the videos should not be considered. The final output should be with the following format:
...
QUESTION_TEMPLATE = """Answer the question: "[QUESTION]" according to the content of the image. Firstly, give a through thinking on this  and then output your thought and analysis process within the <think> </think> tags and provide your score within the <answer> </answer> tags (formatted as Score: b). The final output should be with the following format:
...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def make_conversation_image(self, example):
        ...
        # example_prompt = QUESTION_TEMPLATE_compare.replace("[QUESTION]", example["problem"]["question"])
        content=[{"type": "image", "image": example["video_path"][i],"max_pixels": 1920 * 1080,} for i in range(len(example["video_path"]))]
        content.append({"type": "text", "text": example_prompt})
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if video_inputs==[]:
            video_inputs=None
        # print([x["image_inputs"][0] for x in inputs if "image_inputs" in x])
        # image_inputs = [x["image_inputs"][0][0] for x in inputs if "image_inputs" in x]
        image_inputs = [y[0] for x in inputs if "image_inputs" in x for y in x["image_inputs"]]
        # print(inputs)
        if image_inputs==[]:
            image_inputs=None
        ...
        if self.state.global_step % self.num_iterations == 0:
            ...
            video_grid_thw = prompt_inputs["video_grid_thw"].repeat(self.num_generations, 1)
            ...
            with torch.no_grad():
                if self.num_iterations > 1:
                    old_per_token_logps = self._get_per_token_logps(model, prompt_completion_ids, attention_mask, pixel_values_videos, video_grid_thw)
                    # print(old_per_token_logps)
                    old_per_token_logps=(old_per_token_logps*attention_mask[:,1:])[:, prompt_length - 1:]
                else:
                    old_per_token_logps=None
            buffer_input={"prompt_completion_ids":prompt_completion_ids,"attention_mask":attention_mask,"pixel_values_videos":pixel_values_videos,"video_grid_thw":video_grid_thw,"old_per_token_logps":old_per_token_logps,
            "prompt_length":prompt_length,"advantages":advantages,"ref_per_token_logps":ref_per_token_logps}
            buffer_metrics={"rewards_per_func":rewards_per_func,"completion_mask":completion_mask,"std_grouped_rewards":std_grouped_rewards}

            self._buffered_inputs[self._step % self.args.gradient_accumulation_steps] = buffer_input
            self._buffered_metrics[self._step % self.args.gradient_accumulation_steps] = buffer_metrics
            ...
            input = self._buffered_inputs[self._step % self.args.gradient_accumulation_steps]
            metrics=self._buffered_metrics[self._step % self.args.gradient_accumulation_steps]
            old_per_token_logps=input["old_per_token_logps"]
            per_token_logps = self._get_per_token_logps(model, input["prompt_completion_ids"], input["attention_mask"], input["pixel_values_videos"], input["video_grid_thw"])
            per_token_logps = (per_token_logps*input["attention_mask"][:,1:])[:, input["prompt_length"] - 1 :]
            coef_1 = torch.exp(per_token_logps - old_per_token_logps)
            # print(old_per_token_logps)
            coef_1 = torch.clamp(coef_1, 0, 10)
            # print(coef_1)
            coef_2 = torch.clamp(coef_1, 1 - self.epsilon_low, 1 + self.epsilon_high)
            # print(coef_2)
            per_token_loss1 = coef_1 * input["advantages"].unsqueeze(1)
            # print(per_token_loss1)
            per_token_loss2 = coef_2 * input["advantages"].unsqueeze(1)
            per_token_loss = -torch.min(per_token_loss1, per_token_loss2)
            # print(per_token_loss)
            # print(per_token_loss)
            ref_per_token_logps = input["ref_per_token_logps"]
            per_token_kl = torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1
            # print(per_token_kl)
            per_token_loss = per_token_loss + self.beta *per_token_kl
            completion_mask=metrics["completion_mask"]
            loss = ((per_token_loss * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
            is_clipped = (per_token_loss1 < per_token_loss2).float()
            rewards_per_func=metrics["rewards_per_func"]
            rewards = rewards_per_func.sum(dim=1)
            std_grouped_rewards=metrics["std_grouped_rewards"]
            clip_ratio = (is_clipped * completion_mask).sum() / completion_mask.sum()
        self._step += 1
